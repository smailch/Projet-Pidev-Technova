...
public class UtilisateurService implements IService<Utilisateur> {
    ...
    public static Utilisateur getUtilisateurById(int id) {
        ...
        try (PreparedStatement pst = MyConnection.getInstance().getConnection().prepareStatement(req)) {
            pst.setInt(1, id);
            ...
            if (res.next()) {
                ...
                utilisateur.setRole(Role.valueOf(res.getString("Role")));
                utilisateur.setDateInscription(res.getDate("DateInscription"));
            }
        } catch (SQLException e) {
    ...
    public static List<Utilisateur> getAllCitoyens() {
        List<Utilisateur> citoyens = new ArrayList<>();
        try {
            String req = "SELECT * FROM `utilisateur` WHERE `Role` = 'CITOYEN'";
            Statement st = MyConnection.getInstance().getConnection().createStatement();
            ResultSet res = st.executeQuery(req);
            while (res.next()) {
                Utilisateur u = new Utilisateur();
