...
public class IncidentController {
    ...
    @FXML
    private TableColumn<Incident, Integer> colIda;

    @FXML
    private TableColumn<Incident, String> colDescriptiona;

    @FXML
    private TableColumn<Incident, String> colLocalisationa;
    ...
    @FXML
    private TableColumn<Incident, String> colDateSignalementa;
    // Instance de IncidentService
    private final IncidentService incidentService = new IncidentService();
    private ObservableList<Incident> incidentsList;
    private static final String[] TYPE_INCIDENT_VALUES = {
            "Voirie",
            "Problème de route",
            "Éclairage",
            "Déchets",
    ...
    private void ajouterIncident() {
        try {
            ...
            if (incidentService.incidentExiste(incident, utilisateur.getId())) {
                showAlert("Erreur", "Cet incident a déjà été signalé !");
                return;
            }

            // Ajout de l'incident dans la base de données
            boolean success = incidentService.ajouterIncident(incident);
            if (success) {
                // Réinitialisation des champs après l'ajout
                clearFields();
                showSuccessAlert("Succès", "Incident ajouté avec succès !");
            } else {
                showAlert("Erreur", "Une erreur est survenue lors de l'ajout de l'incident.");
            }
    ...
    private void remplirChampsIncident(Incident incident) {
        if (incident != null) {
            ...
            imagePath = incident.getImage();

            // Mettre à jour l'affichage de l'image si vous avez un `ImageView`
            if (incident.getImage() != null && !incident.getImage().isEmpty()) {
                Image image = new Image("file:" + incident.getImage());
                imageView.setImage(image);
            } else {
    ...
    private void goToajouterIncidentPage(javafx.event.ActionEvent event) {
        ...
        NavigationUtils.switchPage("/ajouterincidentForm.fxml", currentStage, titleBar);
    }
    @FXML
    private void goTolisterIncidentPage(javafx.event.ActionEvent event) {
       {
            Stage currentStage = (Stage) ((javafx.scene.Node) event.getSource()).getScene().getWindow();
            HBox titleBar = NavigationUtils.createCustomTitleBar(currentStage);
            NavigationUtils.switchPage("/listeincidentForm.fxml", currentStage, titleBar);
        }
    }
    @FXML
    private void gohistoriquecitoyen(javafx.event.ActionEvent event) {
        {
            Stage currentStage = (Stage) ((javafx.scene.Node) event.getSource()).getScene().getWindow();
            HBox titleBar = NavigationUtils.createCustomTitleBar(currentStage);
            NavigationUtils.switchPage("/historiquecitoyen.fxml", currentStage, titleBar);
        }
    }
