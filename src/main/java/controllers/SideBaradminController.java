...
public class SideBaradminController {
    ...
    @FXML
    private void onadminIncidentsClick(Event event) {
        Stage currentStage = (Stage) ((javafx.scene.Node) event.getSource()).getScene().getWindow();
        HBox titleBar = NavigationUtils.createCustomTitleBar(currentStage);
        NavigationUtils.switchPage("/adminincidentForm.fxml", currentStage, titleBar);
    }
    @FXML
    private void onadminServiceClick(Event event) {
        Stage currentStage = (Stage) ((javafx.scene.Node) event.getSource()).getScene().getWindow();
        HBox titleBar = NavigationUtils.createCustomTitleBar(currentStage);
        NavigationUtils.switchPage("/serviceintervention.fxml", currentStage, titleBar);
    }
