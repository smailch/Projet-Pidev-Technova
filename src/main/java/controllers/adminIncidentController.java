...
import static services.ServiceInterventionService.envoyerEmailIncidentResolut;
import static services.ServiceInterventionService.EmailTravaux;
public class adminIncidentController {
    ...
    @FXML
    private Label incidentNonTraiteLabel;
    ...
    private void initialize() {
        ...
        incidentTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                incidentService.afficherIncidentsadmine() ;
            }
        });
    }
    private void mettreAJourIncidentNonTraite() {
        int nombreNonTraites = incidentService.getNombreIncidentsNonTraites();
        incidentNonTraiteLabel.setText("Incidents non traités : " + nombreNonTraites);
    }
    public String getServiceNameById(int serviceId) {
        String serviceName = null;
        String req = "SELECT nom_service FROM serviceintervention WHERE id = ?";

        try (PreparedStatement stmt = MyConnection.getInstance().getCnx().prepareStatement(req)) {
    ...
    private void chargerIncidents() {
        incidentsList = FXCollections.observableArrayList(incidentService.afficherIncidentsadmine());
    ...
    private void modifierstatusIncident() {
        ...
        // Si le statut est "Résolu", enregistrer la date et l'heure de résolution
        if (selectedIncident != null) {
            if ("Résolu".equalsIgnoreCase(selectedIncident.getStatut())) {
                Timestamp resolutionTime = new Timestamp(System.currentTimeMillis());
                selectedIncident.setDateResolution(resolutionTime);

                // Mettre à jour dans la base de données
                IncidentService incidentService = new IncidentService();
                incidentService.ajouterTempsResolution(selectedIncident);
                mettreAJourIncidentNonTraite();

            }
        } else {
            System.out.println("Erreur: Incident sélectionné est null.");
        }
        ...
        // Si le statut est "En cours" et le type d'incident est "Problème de route", envoyer un email pour informer les citoyens
        if ("En cours".equalsIgnoreCase(statut)
                && "Problème de route".equalsIgnoreCase(selectedIncident.getTypeIncident())) {
            mettreAJourIncidentNonTraite();
            envoyerEmailTravaux(selectedIncident);

        }

        // Si le statut est "Résolu", envoyer un email au citoyen concerné
        if ("Résolu".equalsIgnoreCase(statut)) {
            mettreAJourIncidentNonTraite();
            int utilisateurId = getUtilisateurIdParIncident(selectedIncident.getId());
            Utilisateur utilisateur = getUserById(utilisateurId);
            if (utilisateur != null && utilisateur.getEmail() != null) {
                envoyerEmailIncidentResolut(utilisateur, selectedIncident);

            }
        }
    }

    private void envoyerEmailTravaux(Incident incident) {
        // Récupérer tous les citoyens
        List<Utilisateur> citoyens = UtilisateurService.getAllCitoyens();

        // Construire le message HTML
        String sujet = "⚠️ Avis de travaux : Problème de route";

        // Envoi de l'email à chaque citoyen
        for (Utilisateur citoyen : citoyens) {
            if (citoyen.getEmail() != null && !citoyen.getEmail().isEmpty()) {
                String message = "<div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; background-color: #f4f4f4; border-radius: 10px;'>"
                        + "<h2 style='color: #e74c3c;'>Avis de travaux : Problème de route ⚠️</h2>"
                        + "<p style='font-size: 16px; color: #333;'>Bonjour <b>" + citoyen.getNom() + "</b>,</p>"
                        + "<p style='font-size: 14px; color: #555;'>"
                        + "Nous vous informons qu'un incident de type <b>'Problème de route'</b> est en cours de traitement dans votre secteur. "
                        + "Les travaux sont actuellement en cours à l'adresse suivante : <b>" + incident.getLocalisation() + "</b>."
                        + "</p>"
                        + "<p style='font-size: 14px; color: #555;'>"
                        + "Afin de minimiser les désagréments, nous vous recommandons vivement d'éviter l'utilisation de votre véhicule dans cette zone."
                        + "</p>"
                        + "<p style='font-size: 14px; color: #555;'>"
                        + "Nous vous encourageons à utiliser des alternatives comme le vélo pour vous déplacer, ce qui permettra de contourner facilement "
                        + "les zones de travaux et d'éviter les embouteillages et retards."
                        + "</p>"
                        + "<p style='font-size: 14px; color: #555;'>"
                        + "Merci de votre compréhension et de votre coopération pour faciliter la gestion de ces travaux."
                        + "</p>"
                        ...
                        + "&copy; 2025 CiviSmart - Tous droits réservés."
                        + "</p>"
                        + "</div>";

                // Appel de la méthode pour envoyer l'email
                EmailTravaux(citoyen.getEmail(), sujet, message);
            }
        }
    }
    ...
    @FXML
    private void afficherDetailsIncident() {
        // Récupérer l'incident sélectionné dans la TableView
        Incident selectedIncident = incidentTable.getSelectionModel().getSelectedItem();

        // Vérifier si un incident est sélectionné
        if (selectedIncident == null) {
            System.out.println("⚠ Aucun incident sélectionné !");
            return;
        }

        // Récupérer directement l'ID de l'utilisateur depuis la base de données
        int utilisateurId = getUtilisateurIdParIncident(selectedIncident.getId());

        if (utilisateurId == 0) {
            System.out.println("⚠ Aucun utilisateur associé trouvé pour cet incident !");
            return;
        }
        ...
        Utilisateur utilisateur = getUserById(utilisateurId);
        if (utilisateur == null) {
            System.out.println("⚠ Impossible de récupérer les informations de l'utilisateur !");
            return;
        }

        // Charger la nouvelle fenêtre (détail de l'incident)
        try {
            // Charger le fichier FXML pour le détail de l'incident
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/incidentDetail.fxml"));
            Parent root = loader.load();

            // Obtenir le contrôleur de la fenêtre de détail
            IncidentDetailController controller = loader.getController();
            controller.initialize(selectedIncident, utilisateur);  // Passer les informations à afficher

            // Créer une nouvelle scène
            Scene scene = new Scene(root);

            // Ouvrir la nouvelle fenêtre dans une nouvelle Stage
            Stage stage = new Stage();
            stage.setTitle("Détails de l'incident");
            stage.setScene(scene);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
            System.out.println("Erreur lors de l'ouverture de la fenêtre des détails de l'incident");
        }
    }

    private static int getUtilisateurIdParIncident(int incidentId) {
        int utilisateurId = 0;
        String query = "SELECT utilisateur_id FROM incident WHERE id = ?";
    ...
    private Utilisateur getUserById(int utilisateurId) {
